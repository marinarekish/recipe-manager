...
### Session behavior
- [x] Frontend stores the token + user in `localStorage` and restores the session on reload.
- [x] Logout clears stored session and returns to the login page.
- [x] Global 401 handling: `authInterceptor` logs the user out and redirects to `/login` on an expired/invalid token, excluding public auth paths (`/api/auth/request-code`, `/api/auth/verify-code`, `/api/auth/register`).
...
- [x] Admin screens are implemented (not placeholders): **Users** (list, role change, delete), **Recipes** (all recipes, search, delete), **Lookups** (Categories / Cuisines / Ingredients tabs, delete).
- [x] Recipe card/grid show loading, error, and empty states; image falls back to a placeholder when `imageUrl` is absent.
- [x] Responsive layout and Material/SCSS consistency at common viewport widths.
- [x] Feedback uses Material snackbars (`NotificationService`) for success/error toasts.

### Deferred (do not mark passing)
- [ ] TODO: `returnUrl` is written by `authGuard` and the 401 interceptor, but the login page does not consume it yet; after login the user is sent to the recipe area. Decide desired deep-link behavior.
- [ ] TODO: frontend tests (Jasmine/Karma) not implemented.
...
- [x] Root `README.md` reflects current code and confirmed decisions.
- [x] `docs/api/api.md` matches the implemented endpoints.
- [x] `docs/auth/auth-flow.md` matches the implemented auth behavior.
- [x] `docs/runbooks/frontend-runbook.md` is up to date with the shipped frontend.
- [x] `docs/runbooks/backend-runbook.md` matches current projects/migrations.
- [x] Known limitations sections are present and accurate (root README updated; re-verify in the next doc pass).
